...
func (uc *UseCase) Execute(ctx context.Context, expressions []executor.Expression, varsToPrint map[string]bool) ([]executor.Item, error) {
	...
	for _, expression := range expressions {
		...
		if _, ok := expressionVars[expression.Variable]; ok {
			return nil, fmt.Errorf("%w: %v", executor.VarIsAlreadyUsed, expression.Variable)
		}
		expressionVars[expression.Variable] = true
		left, leftIsInt := expression.Left.(int)
		right, rightIsInt := expression.Right.(int)
		if leftIsInt && rightIsInt {
			uc.service.RequestCalculation(ctx, left, right, expression.Variable, expression.Operation)
			continue
		}
			...
			if depExpressions := expressionMap[expression.Variable]; depExpressions != nil {
				for _, dep := range depExpressions {
					if dep.Variable == expression.Left.(string) {
						return nil, fmt.Errorf("%wfor variables: %v", executor.CyclicDependency, fmt.Sprintf("%s and %s", expression.Variable, dep.Variable))
					}
				}
			}
	...
	for expressionVar := range expressionMap {
		if _, ok := expressionVars[expressionVar]; !ok {
			return nil, fmt.Errorf("%w: %v", executor.VarNeverBeCalc, expressionVar)
		}
	}

	for variable, varToPrint := range varsToPrint {
		if !varToPrint {
			return nil, fmt.Errorf("%w: %v", executor.VarToPrintNotFound, variable)
		}
	}
	...
	handlerFunc := func(result executor.Result) (stop bool, err error) {
		...
		if varsToPrint[result.Key] {
			items = append(items, executor.Item{Var: result.Key, Value: resultMap[result.Key]})
